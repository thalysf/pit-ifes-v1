...
@Component({
    selector: 'app-responder-extensao',
    templateUrl: './responder-extensao.component.html',
    styleUrls: ['./responder-extensao.component.scss']
})
export class ResponderExtensaoComponent {
    @Input() idPit!: string;
    ...
    private listarAtividadesResponder() {
        ...
        const observerAtividades = this.apiService.get('respostas/extensao/atividades', {
            idUsuario: this.authService.getUserId(),
            idPit: this.idPit
        });

        const observerRespostas = this.apiService.get('respostas/extensao', {idPit: this.idPit});
        ...
        forkJoin([observerAtividades, observerRespostas]).subscribe(
            ([resultadoAtividades, resultadoRespostas]: any) => {
                ...
                atividades.forEach(atividade => {
                    const resposta = respostas.find(resposta => resposta.atividade?.idAtividade == atividade.idAtividade);
    ...
    salvarRespostaAtividade = (resposta: RespostaAtividadeComponente) => {
        ...
        this.apiService.post('respostas/extensao', respostaAtividadeSalvar)
            .subscribe(response => {
